...
unsigned int HoanViPC1(unsigned int K1, unsigned int K2, int cs1, int cs2)
{
	int PC1[] = {57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51,
...
unsigned int DichVong(unsigned int C0, int s)
{
	unsigned int C1;
...
unsigned int HoanViPC2(unsigned int C1, unsigned int D1, int cs1, int cs2)
{
...
void SinhKhoa(unsigned int K1, unsigned int K2, unsigned int key1[16], unsigned int key2[16])
{
	unsigned int C0, D0;
	C0 = HoanViPC1(K1, K2, 0, 28);
	D0 = HoanViPC1(K1, K2, 28, 56);
	...
	for (int i = 0; i < 16; i++)
	{
		C1 = DichVong(C0, s[i]);
		D1 = DichVong(D0, s[i]);
		key1[i] = HoanViPC2(C1, D1, 0, 24);
		key2[i] = HoanViPC2(C1, D1, 24, 48);
		C0 = C1;
		D0 = D1;
	}
}

unsigned int HoanViIP_M(unsigned int M1, unsigned int M2, int cs1, int cs2)
{
...
unsigned int MoRongNuaPhai(unsigned int R0, int cs1, int cs2)
{
...
unsigned int ThayThe_Sbox(unsigned int A1, unsigned int A2)
{
...
unsigned int F(unsigned int L0, unsigned int R0, unsigned int key1, unsigned int key2)
{
	...
	ER01 = MoRongNuaPhai(R0, 0, 24);
	ER02 = MoRongNuaPhai(R0, 24, 48);
...
unsigned int HoanViIP1(unsigned int M1, unsigned int M2, int chiso1, int chiso2)
{
...
int main()
{
	...
	unsigned int K1 = 0x35137844;
	unsigned int K2 = 0x65A003DD;

	printf("Khoa K = %08X %08X\n", K1, K2);

	unsigned int key1[16];
	unsigned int key2[16];
	SinhKhoa(K1, K2, key1, key2);
	printf("\n-------- 16 khoa duoc sinh ra la --------\n");
	for (int i = 0; i < 16; i++)
	{
		printf("Key %d \t= %06X %06X\n", i + 1, key1[i], key2[i]);
	}

	unsigned int M1 = 0x950FB522;
	unsigned int M2 = 0xA6E2B1DB;
	unsigned int L0, R0;
	L0 = HoanViIP_M(M1, M2, 0, 32);
	R0 = HoanViIP_M(M1, M2, 32, 64);
	printf("\n-------- Ket qua hoan vi IP --------\n");
	printf("L0 = %08X\n", L0);
	printf("R0 = %08X\n", R0);
	...
	// C = F20CA5AE 288E3903
	unsigned int C1, C2;
	C1 = HoanViIP1(R1, L1, 0, 32);
	C2 = HoanViIP1(R1, L1, 32, 64);
	printf("-------- Ban ma can tim --------\n");
	printf("C = %08X %08X", C1, C2);
}
